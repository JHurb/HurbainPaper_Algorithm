function simuDetox_DoseRespKO(kovect,kosecond,savedyn)
%% For Regulation KO (+10) or enzyme Knockdown (-1)/OverActivation(+1)
%kovect : parameter to change
%savedyn : save to file 1=on / 2=off
tic

dataname1='mcmcpar_100k';
Result=dlmread(strcat(dataname1,'.dat'),' ',1,0);


dataname2='mcmcfpb_100k';
Result_IC=dlmread(strcat(dataname2,'.dat'),' ',1,0);
param_Benjorder=[15 17 16 2 8 9 10 12 11 14 13 1 3 4 5 6];%[15 17 16 2 8 9 10 12 11 13 14 1 3 4 5 6]
Result_ic=Result_IC(:,param_Benjorder);

nbpar=min(size(Result,1),size(Result_IC,1));

%% Param_simu
reverseStr='';

% paramvar_puismax=2; %puissance de 10 max ([1,+inf[)
% paramvar_nbunit=9; %intervale de l'unit� ([1,9])
% paramvar_center=1; %d�calage par rapport � la puissance de 10 du milieu ([1,paramvar_puismax-1)
% paramvar_matrix=(10.^((1:paramvar_puismax)-floor(paramvar_puismax/2)-paramvar_center)).'*(1:9/paramvar_nbunit:9); %matrix : i=puissance de 10 ; j=unit�
% paramvar_nbr=numel(paramvar_matrix); %nbr total de valeur used
paramvar=10.^([-1:0.1:-0.50 -0.52:0.02:0.48 0.5:0.1:1]);%[sort(reshape(paramvar_matrix,[1,paramvar_nbr])) 10^(paramvar_puismax-floor(paramvar_puismax/2)-paramvar_center+1)]; %vecteur param ordre croissant  -1
paramvar_nbr = numel(paramvar);%paramvar_nbr +1;


tmaxv=[10000 1800]; %Simulation time
nbptsv=[10000 1000]; %nb of point

Fglu=40;
penteIv=[0.0 0.0;0.0 0.0]; % stress slope, microM/s
Imaxv=[0.000 500;Fglu Fglu]; % maximum value of stress
ITimev=[tmaxv(1) tmaxv(2);tmaxv(1) tmaxv(2)]; % bolus time
Tperiodv=[tmaxv(1) tmaxv(2);tmaxv(1) tmaxv(2)]; % repetition periode (>=tmax = off)
stress_noisev=[0 0;0 0]; % percentage of max noise (0% = off), module noise freq with nbpts
starting_stress=[0 0;0 0]; % starting point of stress




%% KO
% KO_nbrpar_inhib=4;

KO_param_x=[0 11 33 46 10 13 20]; % parameter to change
KO_param_val=[10 -1];%param modification
rupturev=numel(KO_param_x(kovect));
% KO_var=zeros(2,Var_nbr,rupturev);
% KO_dxdt=zeros(2,rupturev);

Var_nbr=16;%nb of variable
% x_steady=zeros(2,Var_nbr);
% f_steady=zeros(2,flow_nbr);

DoseResp_5min=zeros(nbpar,(Var_nbr+6)*paramvar_nbr+1,rupturev);
DoseResp_30min=zeros(nbpar,(Var_nbr+6)*paramvar_nbr+1,rupturev);
Hmax=zeros(paramvar_nbr,rupturev);
GSHmax=zeros(paramvar_nbr,rupturev);
NHmax=zeros(paramvar_nbr,rupturev);


if savedyn==1
% nbptsDyn=floor((tmaxv(2)+1)/5);
    tlin=[-1000 0:0.1:10 11:1:50 60:10:200 250:50:300];%0:1:tmaxv(2)
    nbptsDyn=numel(tlin);
    DistriDyn=zeros(nbpar,Var_nbr*nbptsDyn+1,rupturev);
end


for i = 1:nbpar
    param_i=Result(i,1:48);
    ks=0;
    for k=kovect
        %% param change
        param=param_i;
        if k>2
            param(KO_param_x(k))=KO_param_val((k<=4)*1+(k>4)*2)+param(KO_param_x(k));
        end
        
        ks=ks+1;
        if KO_param_x(kosecond(ks))~=0 && KO_param_x(kosecond(ks))~=11
            param(KO_param_x(kosecond(ks)))=KO_param_val((kosecond(ks)<=4)*1+(kosecond(ks)>4)*2)+param(KO_param_x(kosecond(ks)));
        end
        
        delta_dxdt_5m=zeros(2,paramvar_nbr);
        delta_dxdt_30m=zeros(2,paramvar_nbr);
        
        %% Basal simulation
        Fext.IC=Result_ic(i,:);
        Fext.IT=[];
        Fext.Tspan=[];
        Fext=simuDetox_Iinput(Fext, 'Fh2o2', penteIv(1,1), Imaxv(1,1),ITimev(1,1), tmaxv(1), nbptsv(1),Tperiodv(1,1),stress_noisev(1,1),starting_stress(1,1));
        Fext=simuDetox_Iinput(Fext, 'Fgluv', penteIv(2,1), Imaxv(2,1),ITimev(2,1), tmaxv(1), nbptsv(1),Tperiodv(2,1),stress_noisev(2,1),starting_stress(2,1));
            
            
%             [~,x1,~]=simuDetox_reso(param,Fext);
            
            
            
         [t1,x1,f1]=simuDetox_reso(param,Fext);
%             
	      %For g6pd modification
          if KO_param_x(k)==11 || KO_param_x(kosecond(ks))==11
            kg6pd=10^param(10);
            Kig6pd=10^param(11);
            NADPH=x1(end,3);
            
            param(10)=log10(kg6pd/(1+NADPH/Kig6pd));
            
            param(11)=param(11)+10;
          end
            
           
            %Input fonction of Fh2o2 and Fgluv
            Fext.IC=x1(end,:);
            
        for j= 1:paramvar_nbr
            %% Stress simulation
            Fext.IT=[];
            Fext.Tspan=[];
            Imaxs=Imaxv(1,2)*paramvar(j);
            Fext=simuDetox_Iinput(Fext, 'Fh2o2', penteIv(1,2), Imaxs,ITimev(1,2), tmaxv(2), nbptsv(2),Tperiodv(1,2),stress_noisev(1,2),starting_stress(1,2));
            Fext=simuDetox_Iinput(Fext, 'Fgluv',penteIv(2,2), Imaxv(2,2),ITimev(2,2), tmaxv(2), nbptsv(2),Tperiodv(2,2),stress_noisev(2,2),starting_stress(2,2));
            
            
            
            [t2,x2,f2]=simuDetox_reso(param,Fext);
            
            
            tf=[(t1-tmaxv(1));t2];
            xf=[x1;x2];
            %             ff=[f1;f2];
            
            %% Store in to a matrix / DoseResp_5min t=5min / DoseResp_30min t=30min
            %Conc 5 min
            x_steady=[x1(end,:);x2(end,:)];
            %Flow 5min
            f_steady=[f1(end,:);f2(end,:)];
            
            
            x_5min=zeros(2,size(x2,2));
            f_steady_time=300;%5min
            x_5min(1,:)=x_steady(1,:);
            for l=1:size(x2,2)% basal SS + stress 5min
                x_5min(2,l)=interp1(t2,x2(:,l),f_steady_time);
            end
            
            f_5min=zeros(2,size(f2,2));
                        
            f_5min(1,:)=f_steady(1,:);
            for l=1:size(f2,2)% basal SS + stress 5min
                f_5min(2,l)=interp1(t2,f2(:,l),f_steady_time);
            end
            dxdt_5m=[f_5min(:,28)-f_5min(:,1),... 1 h2o2
                2*f_5min(:,2)-2*f_5min(:,1),... 2 gsh
                f_5min(:,5)+f_5min(:,7)+(f_5min(:,3)-f_5min(:,4))-f_5min(:,2),... 3 nh
                f_5min(:,18)-f_5min(:,5)-(f_5min(:,20)-f_5min(:,21)),... 4 g6p
                f_5min(:,5)-f_5min(:,6),... 5 6pgl
                f_5min(:,6)-f_5min(:,7),... 6 6pg
                f_5min(:,7)-(f_5min(:,10)-f_5min(:,11))-(f_5min(:,8)-f_5min(:,9)),... 7 ru5p
                (f_5min(:,8)-f_5min(:,9))-(f_5min(:,12)-f_5min(:,13))-(f_5min(:,16)-f_5min(:,17)),... 8 x5p
                (f_5min(:,10)-f_5min(:,11))-(f_5min(:,12)-f_5min(:,13))-f_5min(:,30),... 9 r5p
                (f_5min(:,12)-f_5min(:,13))-(f_5min(:,14)-f_5min(:,15)),... 10 s7p
                (f_5min(:,14)-f_5min(:,15))-(f_5min(:,16)-f_5min(:,17)),... 11 e4p
                f_5min(:,29)-f_5min(:,18), ... 12 glc
                (f_5min(:,20)-f_5min(:,21))-(f_5min(:,22)-f_5min(:,23))+(f_5min(:,14)-f_5min(:,15))+(f_5min(:,16)-f_5min(:,17)),... 13 f6p
                (f_5min(:,22)-f_5min(:,23))-(f_5min(:,24)-f_5min(:,25)),... 15 fbp
                (f_5min(:,24)-f_5min(:,25))-(f_5min(:,26)-f_5min(:,27)),... 16 dhap
                (f_5min(:,24)-f_5min(:,25))+(f_5min(:,12)-f_5min(:,13))-(f_5min(:,14)-f_5min(:,15))+(f_5min(:,16)-f_5min(:,17))+(f_5min(:,26)-f_5min(:,27))-f_5min(:,31)]; %17 gap
            delta_dxdt_5m(:,j)=sum(abs(dxdt_5m').^2)/2;
            delta_dxdt_5m(:,j)=sqrt(delta_dxdt_5m(:,j));
            
            
            dxdt_30m=[f_steady(:,28)-f_steady(:,1),... 1 h2o2
                2*f_steady(:,2)-2*f_steady(:,1),... 2 gsh
                f_steady(:,5)+f_steady(:,7)+(f_steady(:,3)-f_steady(:,4))-f_steady(:,2),... 3 nh
                f_steady(:,18)-f_steady(:,5)-(f_steady(:,20)-f_steady(:,21)),... 4 g6p
                f_steady(:,5)-f_steady(:,6),... 5 6pgl
                f_steady(:,6)-f_steady(:,7),... 6 6pg
                f_steady(:,7)-(f_steady(:,10)-f_steady(:,11))-(f_steady(:,8)-f_steady(:,9)),... 7 ru5p
                (f_steady(:,8)-f_steady(:,9))-(f_steady(:,12)-f_steady(:,13))-(f_steady(:,16)-f_steady(:,17)),... 8 x5p
                (f_steady(:,10)-f_steady(:,11))-(f_steady(:,12)-f_steady(:,13))-f_steady(:,30),... 9 r5p
                (f_steady(:,12)-f_steady(:,13))-(f_steady(:,14)-f_steady(:,15)),... 10 s7p
                (f_steady(:,14)-f_steady(:,15))-(f_steady(:,16)-f_steady(:,17)),... 11 e4p
                f_steady(:,29)-f_steady(:,18), ... 12 glc
                (f_steady(:,20)-f_steady(:,21))-(f_steady(:,22)-f_steady(:,23))+(f_steady(:,14)-f_steady(:,15))+(f_steady(:,16)-f_steady(:,17)),... 13 f6p
                (f_steady(:,22)-f_steady(:,23))-(f_steady(:,24)-f_steady(:,25)),... 15 fbp
                (f_steady(:,24)-f_steady(:,25))-(f_steady(:,26)-f_steady(:,27)),... 16 dhap
                (f_steady(:,24)-f_steady(:,25))+(f_steady(:,12)-f_steady(:,13))-(f_steady(:,14)-f_steady(:,15))+(f_steady(:,16)-f_steady(:,17))+(f_steady(:,26)-f_steady(:,27))-f_steady(:,31)]; %17 gap
            delta_dxdt_30m(:,j)=sum(abs(dxdt_30m').^2)/2;
            delta_dxdt_30m(:,j)=sqrt(delta_dxdt_30m(:,j));
            
            % half time
%             for l =1:Var_nbr
%                 if x_steady(1,l)<x_steady(2,l)
%                     if abs(max(x2(:,l))-x_steady(2,l))>1e-2
%                         idx=find(x2(:,l)>=(max(x2(:,l)+(x_steady(2,l)-max(x2(:,l)))/2)));
%                         tdemi_idx(j,l)=t2(idx(end));
%                     else
%                         idx=find(x2(:,l)>(x_steady(1,l)+(x_steady(2,l)-x_steady(1,l))/2));
%                         tdemi_idx(j,l)=t2(idx(1));
%                     end
%                         
%                 else
%                     if abs(min(x2(:,l))-x_steady(2,l))>1e-2
%                         idx=find(x2(:,l)<=(min(x2(:,l)+(x_steady(2,l)-min(x2(:,l)))/2)));
%                         tdemi_idx(j,l)=t2(idx(end));
%                     else
%                         idx=find(x2(:,l)<(x_steady(1,l)+(x_steady(2,l)-x_steady(1,l))/2));
%                         tdemi_idx(j,l)=t2(idx(1));
%                     end
%                     
% %                     
% %                     idx=find(x2(:,l)<(x_steady(1,l)+(x_steady(2,l)-x_steady(1,l))/2));
% %                     tdemi_idx(j,l)=t2(idx(1));
%                 end
%             end
%             tdemi(j)=mean(tdemi_idx(j,:));
            
            
            %x_steady_reshape=x_5min';
            DoseResp_5min(i,(1:Var_nbr)+(j-1)*Var_nbr,k)=x_5min(2,:);%x_steady_reshape(:)';
            Hmax(j,k)=max(x2(:,1));
            GSHmax(j,k)=min(x2(:,2));
            NHmax(j,k)=min(x2(:,3));
            
            %x_steady_reshape=x_steady';
            DoseResp_30min(i,(1:Var_nbr)+(j-1)*Var_nbr,k)=x_steady(2,:);%x_steady_reshape(:)';
            
            
            % store dyn
            if savedyn==1
                if paramvar(j)==1
                    [tfuniq,tidx]=unique(tf);
                    for l=1:Var_nbr
                        xlin=interp1(tfuniq,xf(tidx,l),tlin);
                        DistriDyn(i,(1:nbptsDyn)+(l-1)*nbptsDyn,k)=xlin;
                    
                    end
                    DistriDyn(i,end,k)=Result(i,end);
                end
            end
            
        end
        DoseResp_5min(i,((Var_nbr)*paramvar_nbr+1):(Var_nbr+1)*paramvar_nbr,k)=Hmax(:,k);
        DoseResp_5min(i,((Var_nbr+1)*paramvar_nbr+1):(Var_nbr+2)*paramvar_nbr,k)=GSHmax(:,k);
        DoseResp_5min(i,((Var_nbr+2)*paramvar_nbr+1):(Var_nbr+3)*paramvar_nbr,k)=NHmax(:,k);
        DoseResp_5min(i,((Var_nbr+3)*paramvar_nbr+1):(Var_nbr+4)*paramvar_nbr,k)=Imaxv(1,2)*paramvar;
        DoseResp_5min(i,((Var_nbr+4)*paramvar_nbr+1):(Var_nbr+5)*paramvar_nbr,k)= delta_dxdt_5m(1,:);
        DoseResp_5min(i,((Var_nbr+5)*paramvar_nbr+1):(Var_nbr+6)*paramvar_nbr,k)= delta_dxdt_5m(2,:);
        DoseResp_5min(i,end,k)=Result(i,end);
        
        DoseResp_30min(i,((Var_nbr)*paramvar_nbr+1):(Var_nbr+1)*paramvar_nbr,k)=Hmax(:,k);
        DoseResp_30min(i,((Var_nbr+1)*paramvar_nbr+1):(Var_nbr+2)*paramvar_nbr,k)=GSHmax(:,k);
        DoseResp_30min(i,((Var_nbr+2)*paramvar_nbr+1):(Var_nbr+3)*paramvar_nbr,k)=NHmax(:,k);
        DoseResp_30min(i,((Var_nbr+3)*paramvar_nbr+1):(Var_nbr+4)*paramvar_nbr,k)=Imaxv(1,2)*paramvar;
        DoseResp_30min(i,((Var_nbr+4)*paramvar_nbr+1):(Var_nbr+5)*paramvar_nbr,k)= delta_dxdt_30m(1,:);
        DoseResp_30min(i,((Var_nbr+5)*paramvar_nbr+1):(Var_nbr+6)*paramvar_nbr,k)= delta_dxdt_30m(2,:);
        DoseResp_30min(i,end,k)=Result(i,end);
    end
    
    if mod(i,100)==0
        msg=sprintf('%d / %d loops',i,nbpar);
        fprintf([reverseStr,msg]);
        reverseStr=repmat(sprintf('\b'),1, length(msg));
    end
    
end


%% save file
if KO_param_val(2)==-1
    name_file='ko';
else
    name_file='act';
end

KO_name={'ctrl','g6pd','gpi','gapd','g6pd','6pgd','tkt'};
for j_i=1:size(kovect,1)
    j=kovect(j_i);
    if j<=4 && j~=1
        KO_namef=['ki' cell2mat(KO_name(j))];
    else
        KO_namef=['' cell2mat(KO_name(j))];
    end
    if kosecond(j_i)~=1
        KO_namefs=cell2mat(KO_name(kosecond(j_i)));
    else
        KO_namefs='';
    end
        
    dataname11=[dataname1 '_Dosrep5m_' name_file strrep([KO_namef KO_namefs],' ','') '.dat'];
    fileID = fopen(dataname11,'w');
    fprintf(fileID, '%s\n','Result_Analyse_MCMC_DoseRep');
    formatSpec = [repmat('%f ',1,size(DoseResp_5min,2)-1),'%f\n'];
    for i=1:size(DoseResp_5min,1)
        fprintf(fileID, formatSpec, DoseResp_5min(i,:,j));
    end
    fclose(fileID);
    
    dataname13=[dataname1 '_Dosrep30m_' name_file strrep([KO_namef KO_namefs],' ','') '.dat'];
    fileID = fopen(dataname13,'w');
    fprintf(fileID, '%s\n','Result_Analyse_MCMC_DoseRep');
    formatSpec = [repmat('%f ',1,size(DoseResp_30min,2)-1),'%f\n'];
    for i=1:size(DoseResp_30min,1)
        fprintf(fileID, formatSpec, DoseResp_30min(i,:,j));
    end
    fclose(fileID);
    
    
    if savedyn==1
        dataname12=[dataname1 '_Dyn_' name_file strrep([KO_namef KO_namefs],' ','') '.dat'];
        fileID = fopen(dataname12,'w');
        fprintf(fileID, '%s\n','Result_Analyse_MCMC_Dyn');
        formatSpec = [repmat('%f ',1,size(DistriDyn,2)-1),'%f\n'];
        for i=1:size(DistriDyn,1)
            fprintf(fileID, formatSpec,DistriDyn(i,:,j));
        end
        fclose(fileID);
    end
end

toc
end
